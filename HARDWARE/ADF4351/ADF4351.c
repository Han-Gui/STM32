...
//	RCC_AHB1PeriphClockCmd(RCC_ADF4351_CE, ENABLE);
	__HAL_RCC_GPIOF_CLK_ENABLE();           //开启GPIOF时钟,上面四个函数留一个就可以
	
	GPIO_InitStruct.Mode=GPIO_MODE_OUTPUT_PP;//
	GPIO_InitStruct.Speed=GPIO_SPEED_HIGH;//100M
	GPIO_InitStruct.Pull = GPIO_PULLUP;// 上拉
	
	GPIO_InitStruct.Pin=ADF4351_CLK_Pin;
	HAL_GPIO_Init(ADF4351_CLK_GPIO, &GPIO_InitStruct);
	
	GPIO_InitStruct.Pin=ADF4351_DIN_Pin;
	HAL_GPIO_Init(ADF4351_DIN_GPIO, &GPIO_InitStruct);
...
void ADF_DOUT_GPIOInit(void)
{
	...
	GPIO_InitStruct.Pin=ADF4351_DOUT_Pin;
	GPIO_InitStruct.Mode=GPIO_MODE_INPUT;
	GPIO_InitStruct.Speed=GPIO_SPEED_HIGH;
	GPIO_InitStruct.Pull = GPIO_NOPULL;//无上下拉
	
	HAL_GPIO_Init(ADF4351_DOUT_GPIO, &GPIO_InitStruct);
}
